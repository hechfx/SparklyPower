...
class AdvancedDupeIpCommand(val m: SparklyNeonVelocity) : SimpleCommand {
    override fun execute(invocation: SimpleCommand.Invocation) {
        ...
        if (connections.isEmpty()) {
            if (lookUpIp) {
                invocation.source().sendMessage("§cO IP $playerName nunca jogou no servidor!".fromLegacySectionToTextComponent())
            } else {
    ...
    override fun hasPermission(invocation: SimpleCommand.Invocation) = invocation.source().hasPermission("sparklyneonvelocity.advanceddupeip")
}